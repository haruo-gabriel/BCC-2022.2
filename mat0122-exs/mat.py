...
def equal(A, B):
    ...
    for d0 in A.D[0]:
        for d1 in A.D[1]:
            if getitem(A, (d0,d1)) != getitem(B, (d0,d1)):
                return False
    return True
...
def add(A, B):
    ...
    #procedure with zero values ommited
    for d0 in A.D[0]:
        for d1 in A.D[1]:
            if getitem(A, (d0,d1)) != 0 or getitem(B, (d0,d1)) != 0:
                dikt[(d0,d1)] = getitem(A, (d0,d1)) + getitem(B, (d0,d1))
    return Mat(A.D, dikt)

    #procedure with zero values not ommited
    #return Mat(A.D, {(d0,d1):getitem(A, (d0,d1))+getitem(B, (d0,d1)) for d0 in A.D[0] for d1 in A.D[1]} ) 
...
def scalar_mul(M, x):
    ...
    if x == 0:
        ...
        return Mat(M.D, {rc : x*M.f[rc] for rc in M.f})
...
def transpose(M):
    ...
    True
    """
    return Mat((M.D[1], M.D[0]), {(d1,d0):M.f[(d0,d1)] for (d0,d1) in M.f})
